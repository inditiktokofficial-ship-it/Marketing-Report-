      ...
      {!report && !loading && (
        <div style={{ maxWidth: 760, margin: "80px auto", padding: "0 24px", textAlign: "center" }}>
          <div style={{ display: "inline-block", background: "rgba(0,255,178,0.08)", border: "1px solid rgba(0,255,178,0.2)", padding: "5px 16px", borderRadius: 20, marginBottom: 28 }}>
            <span style={{ color: ACCENT, fontSize: 11, letterSpacing: 2, textTransform: "uppercase", fontFamily: "Syne, sans-serif" }}>AI-Powered · No Install Required</span>
          </div>
          <h1 style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 48, lineHeight: 1.1, marginBottom: 16 }}>
            Full Marketing Audit<br />
            <span style={{ background: `linear-gradient(90deg, ${ACCENT}, #00A8FF)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>in 60 Seconds</span>
          </h1>
          <p style={{ color: "#64748B", fontSize: 16, marginBottom: 48, lineHeight: 1.7 }}>
            Enter any website URL to get a complete marketing audit, competitor analysis, and 90-day growth strategy — then export a professional PDF for your client.
          </p>

          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 32 }}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, color: "#64748B", textTransform: "uppercase", letterSpacing: 1, fontFamily: "Syne, sans-serif", marginBottom: 8, textAlign: "left" }}>Website URL</label>
              <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === "Enter" && runAnalysis()}
                placeholder="https://example.com"
                style={{ width: "100%", background: "#0A0E1A", border: `1px solid ${BORDER}`, borderRadius: 10, padding: "14px 16px", color: "#E2E8F0", fontSize: 15, fontFamily: "DM Sans, sans-serif" }} />
            </div>
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: "block", fontSize: 12, color: "#64748B", textTransform: "uppercase", letterSpacing: 1, fontFamily: "Syne, sans-serif", marginBottom: 8, textAlign: "left" }}>Industry (optional)</label>
            ...
            <button onClick={runAnalysis}
              style={{ width: "100%", background: `linear-gradient(135deg, ${ACCENT}, #00C8A8)`, color: DARK, border: "none", borderRadius: 10, padding: "16px", fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 15, cursor: "pointer", letterSpacing: 0.5 }}>
              Run Full Analysis →
            </button>
          </div>

          <div style={{ display: "flex", justifyContent: "center", gap: 32, marginTop: 48 }}>
            {[["🔍", "Website Audit"], ["⚔️", "Competitor Intel"], ["📈", "Growth Strategy"], ["📄", "PDF Export"]].map(([icon, label]) => (
              <div key={label} style={{ textAlign: "center" }}>
                <div style={{ fontSize: 24, marginBottom: 8 }}>{icon}</div>
                <div style={{ fontSize: 12, color: "#334155", fontFamily: "Syne, sans-serif", letterSpacing: 0.5 }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {loading && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "60vh", gap: 24 }}>
          <div style={{ position: "relative", width: 80, height: 80 }}>
            ...
            <div className="pulse" style={{ position: "absolute", inset: 0, borderRadius: "50%", border: `3px solid transparent`, borderTopColor: ACCENT, animation: "spin 1s linear infinite" }} />
            <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 18, marginBottom: 8 }}>Analyzing…</div>
            <div className="pulse" style={{ fontSize: 14, color: "#64748B" }}>{step}</div>
          </div>
        </div>
      )}
      ...
      {report && !loading && (
        <div className="fade-in" style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
            <div>
              <div style={{ fontSize: 12, color: "#64748B", textTransform: "uppercase", letterSpacing: 1, fontFamily: "Syne, sans-serif", marginBottom: 4 }}>Analysis Complete</div>
              <div style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 20, color: ACCENT }}>{url}</div>
            </div>
            <button onClick={() => { setReport(null); setUrl(""); setIndustry(""); }}
              style={{ background: "transparent", border: `1px solid ${BORDER}`, color: "#64748B", borderRadius: 8, padding: "8px 16px", fontFamily: "Syne, sans-serif", fontSize: 13, cursor: "pointer" }}>
              ← New Analysis
            </button>
          </div>

          {/* Tabs */}
          <div style={{ display: "flex", gap: 4, background: CARD, padding: 6, borderRadius: 12, border: `1px solid ${BORDER}`, marginBottom: 32, width: "fit-content" }}>
            {[["audit", "Website Audit"], ["competitors", "Competitors"], ["strategy", "90-Day Strategy"]].map(([t, label]) => (
          ...
          {tab === "audit" && (
            <div>
              {/* Score overview */}
              <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 32, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 32, marginBottom: 24 }}>
                <div style={{ textAlign: "center" }}>
                  <ScoreRing score={report.audit.overallScore} size={100} />
                  <div style={{ fontSize: 12, color: "#64748B", marginTop: 8, fontFamily: "Syne, sans-serif", letterSpacing: 0.5 }}>Overall</div>
                </div>
                <div>
                  <Bar label="SEO" value={report.audit.scores.seo} />
                  <Bar label="Performance" value={report.audit.scores.performance} />
                  <Bar label="Content Quality" value={report.audit.scores.content} />
                  <Bar label="UX / Conversion" value={report.audit.scores.ux} />
                </div>
              </div>

              {/* Summary */}
              <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 28, marginBottom: 24 }}>
                <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 16, marginBottom: 12 }}>Executive Summary</h3>
                <p style={{ color: "#94A3B8", lineHeight: 1.8, fontSize: 14 }}>{report.audit.summary}</p>
              </div>

              {/* Strengths */}
              <div style={{ background: CARD, border: `1px solid rgba(0,255,178,0.15)`, borderRadius: 16, padding: 28, marginBottom: 24 }}>
                <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 16, marginBottom: 16, color: ACCENT }}>✦ Strengths</h3>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                  {report.audit.strengths.map((s, i) => (
                    <div key={i} style={{ background: "rgba(0,255,178,0.08)", border: "1px solid rgba(0,255,178,0.2)", borderRadius: 8, padding: "8px 14px", fontSize: 13, color: ACCENT }}>{s}</div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: "hidden" }}>
                <div style={{ padding: "20px 28px", borderBottom: `1px solid ${BORDER}` }}>
                  <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 16 }}>Issues & Recommendations</h3>
                </div>
                {report.audit.issues.map((issue, i) => {
                  const s = severityStyle[issue.severity] || severityStyle.Low;
                  return (
                    <div key={i} style={{ padding: "18px 28px", borderBottom: i < report.audit.issues.length - 1 ? `1px solid ${BORDER}` : "none", display: "grid", gridTemplateColumns: "1fr auto 2fr", gap: 20, alignItems: "start" }}>
                      <div style={{ fontWeight: 500, fontSize: 14 }}>{issue.issue}</div>
                      <div style={{ background: s.bg, color: s.color, padding: "3px 12px", borderRadius: 20, fontSize: 11, fontFamily: "Syne, sans-serif", fontWeight: 700, whiteSpace: "nowrap" }}>{issue.severity}</div>
                      <div style={{ fontSize: 13, color: "#64748B", lineHeight: 1.6 }}>{issue.recommendation}</div>
                    </div>
                  );
                })}
              </div>
          ...
          {tab === "competitors" && (
            <div>
              <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 28, marginBottom: 24 }}>
                <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 16, marginBottom: 12 }}>Competitive Landscape</h3>
                <p style={{ color: "#94A3B8", lineHeight: 1.8, fontSize: 14 }}>{report.competitors.overview}</p>
              </div>
              <div style={{ display: "grid", gap: 16 }}>
                {report.competitors.list.map((c, i) => (
                  <div key={i} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 28 }}>
                    <div style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: 18, marginBottom: 20 }}>{c.name}</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }}>
                      {[["💪 Strengths", c.strengths, "#86EFAC"], ["⚠️ Weaknesses", c.weaknesses, "#FCA5A5"], ["🎯 Your Opportunity", c.opportunity, ACCENT]].map(([label, val, col]) => (
                        <div key={label}>
                          <div style={{ fontSize: 12, fontFamily: "Syne, sans-serif", letterSpacing: 0.5, marginBottom: 8, color: col }}>{label}</div>
          ...
          {tab === "strategy" && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20, marginBottom: 24 }}>
                {["month1", "month2", "month3"].map((m, i) => (
                  <div key={m} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 24 }}>
                    <div style={{ color: ACCENT, fontFamily: "Syne, sans-serif", fontSize: 11, letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Month {i + 1}</div>
                    <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 16, marginBottom: 16 }}>{report.strategy[m].title}</h3>
                    {report.strategy[m].actions.map((a, j) => (
              ...
              <div style={{ background: CARD, border: `1px solid rgba(0,255,178,0.2)`, borderRadius: 16, padding: 28 }}>
                <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: 16, marginBottom: 20 }}>⚡ Quick Wins — Start Today</h3>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  {report.strategy.quickWins.map((w, i) => (
                    <div key={i} style={{ display: "flex", gap: 10, fontSize: 13, color: "#94A3B8", alignItems: "flex-start" }}>
                      <span style={{ color: ACCENT, fontSize: 16, lineHeight: 1.3 }}>✦</span>{w}
                    </div>
                  ))}
                </div>
              </div>
